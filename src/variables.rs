...
impl VariableCompletion for String{
    ...
    fn set_single_variable(&mut self, key:&str,value:&str)->Result<String> {
        ...
        if &new==self{
            return Err(format_err!("Variable could not be set"))
        }else{
            *self=new;
...
impl VariableCompletion for PathBuf{
    fn required_variables(&self)->Result<Vec<String>> {
        self.to_str().context("Can not parse path as String")?.to_string().required_variables()
    }
    fn set_single_variable(&mut self, key:&str,value:&str)->Result<PathBuf> {
        let mut string=self.to_str().context("Can not parse path as String")?.to_string();
...
impl VariableCompletion for FileSource{
    fn required_variables(&self)->Result<Vec<String>> {
        match self {
            ...
            FileSource::Borg { archive, backup_id, path }=>{
                let rb_archive=archive.to_owned().required_variables()?;
                let rb_path=path.to_owned().required_variables()?;
            ...
            FileSource::Local { path }=>{
                path.to_owned().required_variables()
            },
    ...
    fn set_single_variable(&mut self, key:&str,value:&str)->Result<FileSource> {
        *self=match self{
            FileSource::Archive { archive, path }=>{
                FileSource::Archive{archive:archive.set_single_variable(key, value)?,path:path.set_single_variable(key, value)?}
            },
            FileSource::Borg { archive, backup_id, path }=>{
                FileSource::Borg { archive: archive.set_single_variable(key, value)?, backup_id: backup_id.set_single_variable(key, value)?, path: path.set_single_variable(key, value)? }
            },
            FileSource::Download { url }=>{
                FileSource::Download { url: url.set_single_variable(key, value)? }
            },
            FileSource::Git { repo, commit, path }=>{
                FileSource::Git { repo: repo.set_single_variable(key, value)?, commit: commit.set_single_variable(key, value)?, path: path.set_single_variable(key, value)? }
            },
            FileSource::Local { path }=>{
                FileSource::Local { path: path.set_single_variable(key, value)? }
            },
            FileSource::Text { content }=>{
                FileSource::Text { content: content.set_single_variable(key, value)? }
            }
        };
        return Ok(self.clone());
    }
...
impl<T> VariableCompletion for Vec<T> where T:VariableCompletion{
    fn required_variables(&self)->Result<Vec<String>> {
        let mut req=vec!();
        for v in self{
...
impl VariableCompletion for File{
    ...
    fn set_single_variable(&mut self, key:&str,value:&str)->Result<Self> {
        Ok(File { path: self.path.set_single_variable(key, value)?, tags: self.tags.clone(), hash:self.hash.clone(), sources: self.sources.set_single_variable(key, value)? })
    }
}
...
mod test{
    use super::*;
    #[test]
    fn make_variable_example(){
        let file=File{
            tags:None,path:"some/path".into(),hash:None,sources:vec!(FileSource::Text { content: "bla".into() })
        };
